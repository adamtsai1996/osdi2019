...
void sched_yield(void) {
	extern Task tasks[];
	int i, no_runnable=1, index=thiscpu->cpu_task->rq_index;
	Runqueue *rq =  &thiscpu->cpu_rq;
	for( i=0; i<rq->nr; i++) {
		index = (index+1)%(rq->nr);
		if( index==0 ) continue;
		if( tasks[rq->runq[index]].state == TASK_RUNNABLE ) {
			no_runnable = 0;
			break;
		}
	}
	if(no_runnable) index=0;
	thiscpu->cpu_task = &tasks[rq->runq[index]];
